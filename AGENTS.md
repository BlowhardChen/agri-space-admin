...
本项目是“农域后台管理系统”的前端管理端，当前代码基座是一个基于 React 的后台模板，并已经接入了农业项目自己的品牌文案与部分业务方向。
...
当前仓库更像“农业场景下的管理后台前端骨架”，而不是已经成型的 GIS 专项平台。因此不要默认这里已经存在地图引擎、地块绘制、设备轨迹等完整能力，除非你先在代码里确认它们真的存在。
...
- 如果未来要接 GIS，先确认坐标系、地图 SDK、交互模式，再决定是否引入地图栈
